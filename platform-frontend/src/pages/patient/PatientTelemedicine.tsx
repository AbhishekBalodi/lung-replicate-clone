        ...
        <Card className="cursor-pointer hover:shadow-md transition-shadow">
          <CardContent className="pt-6">
            <div className="flex flex-col items-center text-center gap-3">
              ...
              <p className="text-sm text-muted-foreground">Face-to-face virtual visit</p>
        ...
        <Card className="cursor-pointer hover:shadow-md transition-shadow">
          <CardContent className="pt-6">
            <div className="flex flex-col items-center text-center gap-3">
              ...
              <p className="text-sm text-muted-foreground">Share reports during call</p>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant={session.session_type === "video" ? "default" : "secondary"}>
                        {session.session_type === "video" ? <><Video className="h-3 w-3 mr-1" />Video</> : <><MessageSquare className="h-3 w-3 mr-1" />Chat</>}
                      </Badge>
                    </div>
                    <div className="flex items-center gap-4 text-sm text-muted-foreground">
                      <span className="flex items-center gap-1"><Calendar className="h-4 w-4" />{new Date(session.scheduled_date).toLocaleDateString()}</span>
                      <span className="flex items-center gap-1"><Clock className="h-4 w-4" />{session.scheduled_time}</span>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <Button variant="outline" size="sm">Reschedule</Button>
                    <Button size="sm" disabled={!session.meeting_link}><Video className="h-4 w-4 mr-1" />Join</Button>
                  </div>
                </div>
              ))}
            </div>
